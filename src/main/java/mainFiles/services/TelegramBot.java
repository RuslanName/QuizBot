...
public class TelegramBot extends TelegramLongPollingBot {
                ...
                if (message.hasText()) {
                    ...
                    if (text.equals("/start") || text.equals("/registration")) {
                        if (usersRepository.existsById(chatId)) {
                            ...
                            sendMessage(chatId, "Здравствуйте! Вам нужно зарегистрироваться. Введите свой Betboom ID");
                        }
                    }
                        ...
                        if (leaderboardUserResults.isEmpty()) {
                            ...
                            for (UserResult userResult : leaderboardUserResults) {
                                leaderboardMessage.append(String.format("%d) @%s - %d правильных; Время: %.2f с.\n",
                                        i++, usersRepository.findById(userResult.getChatId()).get().getUserName(), userResult.getResult(), userResult.getTime()));
                            }
            ...
            if (isUserInChannel(chatId)) {
                ...
                if (differentStatesRepository.findById(chatId).get().getState() == ActionType.QUIZ_START_CREATE_QUESTION.getCode()) {
                    ...
                    if (callbackData.matches("ANSWER_OPTION_\\d+_BUTTON")) {
                        ...
                        QuizState quizState = quizStatesRepository.findById(getNextIdForChatId("quiz_states_data", chatId) - 1)
                                .orElseThrow();
    ...
    private void start(long chatId) {
        if (isOwner(chatId)) {
            ...
            keyboardRows.add(createKeyboardRow(QUIZ_CREATE_KEYBOARD, SHOW_USERS_LEADERBOARD_KEYBOARD));
    ...
    private void registration(Message message) {
        ...
        if (isNumeric(text)) {
            ...
            user.setBetboomId(Long.valueOf(text));
